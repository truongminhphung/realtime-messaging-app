...
class NotificationSystemTester:
    ...
    async def run_comprehensive_tests(self) -> Dict[str, Any]:
        ...
        results = {"test_start": datetime.utcnow().isoformat(), "tests": {}}

        try:
            # Test 1: Basic notification creation
            results["tests"][
                "basic_creation"
            ] = await self._test_basic_notification_creation()

            # Test 2: Message notification integration
            results["tests"][
                "message_integration"
            ] = await self._test_message_notification_integration()

            # Test 3: Room invite integration
            results["tests"][
                "room_invite_integration"
            ] = await self._test_room_invite_integration()

            # Test 4: Friend request integration
            results["tests"][
                "friend_request_integration"
            ] = await self._test_friend_request_integration()

            # Test 5: Notification queries and filtering
            results["tests"][
                "query_filtering"
            ] = await self._test_notification_queries()

            # Test 6: Notification preferences
            results["tests"][
                "preferences"
            ] = await self._test_notification_preferences()
        ...
        try:
            async with get_db_session() as session:
                ...
                notifications = await NotificationService.get_user_notifications(
                    session=session, user_id=self.test_user_id, limit=10, offset=0
                )

                # Test marking as read
                success = await NotificationService.mark_as_read(
                    session=session, notification_id=notification.id
                )
        ...
        try:
            async with get_db_session() as session:
                ...
                all_notifications = await NotificationService.get_user_notifications(
                    session=session, user_id=self.test_user_id, limit=50, offset=0
                )
                ...
                total_count = await NotificationService.get_notification_count(
                    session=session, user_id=self.test_user_id
                )

                unread_count = await NotificationService.get_notification_count(
                    session=session, user_id=self.test_user_id, unread_only=True
                )
        ...
        try:
            ...
            async with get_db_session() as session:
                ...
                successful_creates = sum(
                    1 for n in notifications if not isinstance(n, Exception)
                )

                # Test batch retrieval
                retrieved_notifications = (
                    await NotificationService.get_user_notifications(
                        session=session, user_id=self.test_user_id, limit=20, offset=0
                    )
                )
            ...
            return {
                ...
                "avg_create_time_ms": (
                    (duration * 1000) / 10 if successful_creates > 0 else 0
                ),
            }
    ...
    try:
        ...
        async with get_db_session() as session:
            ...
            count = await NotificationService.get_notification_count(
                session=session, user_id=uuid4()  # Non-existent user should return 0
            )
