...
async def test_authentication():
    ...
    async with httpx.AsyncClient() as client:
        ...
        if response.status_code == 200:
            ...
            print(
                f"❌ User profile retrieval failed: {response.status_code} - {response.text}"
            )
        print()
        ...
        if response.status_code == 401:
            ...
            print(
                f"❌ Should have rejected request without token: {response.status_code}"
            )
        print()
        ...
        if response.status_code == 200:
            ...
            if response.status_code == 401:
                ...
                print(
                    f"❌ Token should be invalid after logout: {response.status_code}"
                )
        else:
        ...
        wrong_login_data = {"email": test_email, "password": "wrongpassword"}
