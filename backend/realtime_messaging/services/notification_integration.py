...
async def create_message_notification(
    ...
    try:
        ...
        if success:
            logger.info(
                f"Successfully published message notification for message {message_id}"
            )
            return True
        else:
            # Fallback: create notifications directly in database
            logger.warning(
                f"Failed to publish to RabbitMQ, creating notifications directly"
            )
...
async def create_room_invite_notification(
    ...
    try:
        ...
        success = await rabbitmq_service.publish_message_notification(
            invite_data, routing_key="notification.room.invite"
        )

        if success:
            logger.info(
                f"Successfully published room invite notification for room {room_id}"
            )
            return True
        else:
            # Fallback: create notification directly
            logger.warning(
                f"Failed to publish room invite to RabbitMQ, creating notification directly"
            )
...
async def create_friend_request_notification(
    ...
    try:
        ...
        # Publish to RabbitMQ
        success = await rabbitmq_service.publish_message_notification(
            friend_request_data, routing_key="notification.friend.request"
        )
        ...
        if success:
            ...
            # Fallback: create notification directly
            logger.warning(
                f"Failed to publish friend request to RabbitMQ, creating notification directly"
            )
    ...
    try:
        ...
        for recipient_id in recipient_ids:
            ...
            try:
                ...
                logger.error(
                    f"Failed to create notification for user {recipient_id}: {e}"
                )

        logger.info(
            f"Created {success_count}/{len(recipient_ids)} notifications as fallback"
        )
...
async def get_user_notification_summary(
    ...
    try:
        # Get total counts
        total_count = await NotificationService.get_notification_count(session, user_id)
...
async def mark_message_notifications_as_read(
    session: AsyncSession, user_id: UUIDType, room_id: UUIDType
) -> int:
