...
async def mark_all_notifications_as_read(
    notification_type: Optional[NotificationType] = Query(
        None, description="Mark all notifications of this type as read"
    ),
    session: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
...
async def delete_all_notifications(
    notification_type: Optional[NotificationType] = Query(
        None, description="Delete all notifications of this type"
    ),
...
async def update_notification_preferences(
    ...
    try:
        ...
        updated_preferences = await NotificationService.update_user_preferences(
            session=session, user_id=current_user.user_id, preferences=preferences_data
        )
