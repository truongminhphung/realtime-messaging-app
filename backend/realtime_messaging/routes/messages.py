...
@router.post(
    "/", response_model=MessageCreateResponse, status_code=status.HTTP_201_CREATED
)
...
async def get_room_messages(
    ...
    session: AsyncSession = Depends(get_db),
    limit: int = Query(
        default=50, le=100, ge=1, description="Number of messages to retrieve"
    ),
...
async def get_message(
    ...
    try:
        ...
        # Get message with sender info
        messages = await MessageService.get_room_messages(
            session, message.room_id, limit=1000, use_cache=False
        )
...
async def delete_message(
    ...
    try:
        success = await MessageService.delete_message(
            session, message_id, current_user.user_id
        )
...
async def search_room_messages(
    ...
    limit: int = Query(
        default=20, le=50, ge=1, description="Number of results to return"
    ),
) -> List[MessageWithSenderInfo]:
...
@router.get(
    "/rooms/{room_id}/after/{message_id}", response_model=List[MessageWithSenderInfo]
)
async def get_messages_after(
    ...
    try:
        ...
        messages = await MessageService.get_messages_after(
            session, room_id, message_id, limit
        )
        return messages
...
async def get_rate_limit_info(current_user: CurrentUser) -> RateLimitInfo:
