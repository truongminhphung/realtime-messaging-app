        ...
        async for session in sessionmanager.get_session():
            ...
            try:
                room = await RoomService.create_room(
                    session, room_data, creator.user_id
                )
            ...
            participants = await RoomService.get_room_participants(
                session, room.room_id, use_cache=False
            )
            print(f"✅ Initial participants ({len(participants)}):")
            for p in participants:
                print(
                    f"   - {p['username']} ({p['display_name']}) joined {p['joined_at']}"
                )
            print()

            # 5. Test joining room
            print("5. Testing room join...")
            join_success = await RoomService.join_room(
                session, room.room_id, participant.user_id
            )
            if join_success:
            ...
            join_again = await RoomService.join_room(
                session, room.room_id, participant.user_id
            )
            if join_again:
                print(
                    f"✅ Duplicate join handled correctly (user was already a participant)"
                )
                print()

            # 7. Test checking participant status
            print("7. Testing participant status checks...")
            is_creator_participant = await RoomService.is_user_participant(
                session, room.room_id, creator.user_id
            )
            is_participant_participant = await RoomService.is_user_participant(
                session, room.room_id, participant.user_id
            )
            is_invitee_participant = await RoomService.is_user_participant(
                session, room.room_id, invitee.user_id
            )
            ...
            participant_rooms = await RoomService.get_user_rooms(
            ...
            leave_success = await RoomService.leave_room(
                session, room.room_id, participant.user_id
            )
            if leave_success:
            ...
            participants_1 = await RoomService.get_room_participants(
                session, room.room_id, use_cache=True
            )
            # Second call (from cache)
            participants_2 = await RoomService.get_room_participants(
                session, room.room_id, use_cache=True
            )
            ...
            print(
                f"   Creator token (first 50 chars): {creator_tokens['access_token'][:50]}..."
            )
            print(
                f"   Participant token (first 50 chars): {participant_tokens['access_token'][:50]}..."
            )
            print()
