    ...
    try:
        async for session in sessionmanager.get_session():
            ...
            try:
                ...
                profile = await UserService.get_user_profile_summary(
                ...
                updated_user = await UserService.update_user_profile(
                    session, new_user.user_id, update_data
                )
                if updated_user:
                ...
                try:
                    update_data = UserUpdate(
                        profile_picture_url="https://example.com/" + "x" * 250
                    )
                    await UserService.update_user_profile(
                        session, new_user.user_id, update_data
                    )
                ...
                print(
                    f"   User Info: {tokens['user']['username']} ({tokens['user']['email']})"
                )
                print()

                # 9. Final profile summary
                print("9. Final profile summary...")
                final_profile = await UserService.get_user_profile_summary(
                    session, new_user.user_id
                )
                if final_profile:
                        ...
                        json.dumps(
                            {
                                ...
                                "profile_picture_url": final_profile[
                                    "profile_picture_url"
                                ],
...
async def demo_api_usage():
    ...
    print("   PATCH /users/profile")
    print(
        "   Body:", json.dumps({"profile_picture_url": "https://example.com/pic.jpg"})
    )
    print()
