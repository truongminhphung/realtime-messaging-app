...
def test_message_validation():
    ...
    try:
        valid_message = MessageCreate(
            room_id=room_id, sender_id=sender_id, content="This is a valid message"
        )
    ...
    try:
        MessageCreate(room_id=room_id, sender_id=sender_id, content="")
    ...
    try:
        MessageCreate(room_id=room_id, sender_id=sender_id, content="   \n\t   ")
        print(
            "❌ Whitespace-only content test failed - should have raised ValidationError"
        )
    except ValidationError as e:
    ...
    long_content = "a" * 501
    try:
        MessageCreate(room_id=room_id, sender_id=sender_id, content=long_content)
    ...
    try:
        message_with_whitespace = MessageCreate(
            room_id=room_id, sender_id=sender_id, content="  Hello World  \n"
        )
    ...
    try:
        message_500 = MessageCreate(
            room_id=room_id, sender_id=sender_id, content=exact_500_chars
        )
    ...
    try:
        valid_internal = MessageCreateInternal(
            room_id=room_id, sender_id=sender_id, content="Internal message content"
        )
    ...
    try:
        MessageCreateInternal(room_id=room_id, sender_id=sender_id, content="")
        print(
            "❌ Invalid internal message test failed - should have raised ValidationError"
        )
    except ValidationError as e:
