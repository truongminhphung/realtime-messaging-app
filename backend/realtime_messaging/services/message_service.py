...
from realtime_messaging.models.message import Message, MessageCreateInternal, MessageGet, MessageWithSenderInfo
...
class MessageService:
    ...
    @staticmethod
    async def get_room_message_count(
    ...
    async def get_messages_after(
        ...
        ref_stmt = select(Message.created_at).where(Message.message_id == after_message_id)
        ...
        stmt = (
            ...
            .where(
                and_(
                    Message.room_id == room_id,
                    Message.created_at > ref_timestamp
                )
            )
    ...
    async def create_message_notification(
        ...
        try:
            ...
            if success:
                logger.info(f"Published notification for message {message.message_id} to RabbitMQ")
            else:
                logger.error(f"Failed to publish notification for message {message.message_id}")
