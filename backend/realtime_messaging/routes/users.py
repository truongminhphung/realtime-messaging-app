...
from realtime_messaging.models.user import UserCreate, UserGet, UserUpdate
...
async def get_user_profile(current_user: CurrentUser) -> UserGet:
    """Get current authenticated user profile."""
    return UserGet.model_validate(current_user)


@router.patch("/profile", response_model=UserGet)
async def update_user_profile(
    user_data: UserUpdate,
    current_user: CurrentUser,
    session: AsyncSession = Depends(get_db),
) -> UserGet:
    """Update current authenticated user profile (partial update)."""
    try:
        user = await UserService.update_user_profile(
            session, current_user.user_id, user_data
        )
        if not user:
            raise HTTPException(
...
async def get_current_user_profile(current_user: CurrentUser) -> UserGet:
    """Get current authenticated user profile (deprecated - use /profile)."""
    return UserGet.model_validate(current_user)
...
async def update_current_user_profile(
    ...
    """Update current authenticated user profile (deprecated - use PATCH /profile)."""
    try:
...
async def get_user_by_id(
    user_id: UUIDType, session: AsyncSession = Depends(get_db)
) -> UserGet:
