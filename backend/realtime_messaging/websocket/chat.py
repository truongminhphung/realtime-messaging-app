...
from realtime_messaging.models.message import MessageCreateInternal, MessageWithSenderInfo
...
async def websocket_endpoint(websocket: WebSocket, room_id: str):
    ...
    try:
        ...
        async for session in sessionmanager.get_session():
            is_participant = await RoomService.is_user_participant(session, room_uuid, user.user_id)
            if not is_participant:
                await websocket.close(code=status.WS_1008_POLICY_VIOLATION, reason="Not a room participant")
                return
            break
